...
if choose=="SIC" or choose == "sic":
    ...
    for line in prog.readlines():
        ...
        if (whole_line[0] == "H"):
            startadrr=whole_line[2]
            length=whole_line[3]
            # endaddr=whole_line[2]+whole_line[3]
        if whole_line[0] == "T":
            whole_line[1] = hex((int(whole_line[1],16)))
            ...
            for x in range(3,len(whole_line)):
                for n in range(0,len(whole_line[x])-1,2):
    ...
    for line in modified_file.readlines():
        ...
        if whole_line[0] == "M":
            ...
            if whole_line[3][0] == "-":
                ...
                if whole_line[2] == "05":
                    modified_addr = address_code[whole_line[1]][1]+second_addr+third_addr
                    addr_concat   = tohex(-int(addr_concat,16),32)
                    addr_concat = addr_concat.replace('0x','')
